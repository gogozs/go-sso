...
import (
	...
	"go-sso/internal/service/controller/auth"
	"go-sso/internal/service/handlers/viewset"
	"go-sso/internal/service/middlewares"
...
func (a *AuthViewset) Login(c *gin.Context) (err error) {
	...
	if err != nil {
		log.Error(err.Error())
		a.FailResponse(c, apierror.ErrInvalid)
...
func (a *AuthViewset) Register(c *gin.Context) (err error) {
	...
	if len(errs) > 0 {
		a.FailResponse(c, apierror.ErrInvalid, errs)
		return
	}
...
func (a *AuthViewset) CheckTelephoneValid(c *gin.Context) (err error) {
	...
	if err := auth.CheckTelephoneValid(telephone); err != nil {
		a.FailResponse(c, apierror.ErrInvalid, err)
		return err
	}
